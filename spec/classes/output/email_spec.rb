...
describe 'cabot::output::email' do
  ...
  context "ubuntu" do
      ...
      it { expect(exported_resources).to contain_ini_setting("cabot_development_SES_USER").with_ensure('absent') }
      it { expect(exported_resources).to contain_ini_setting("cabot_development_SES_PASS").with_ensure('absent') }
    end
